  ...
  convertAddressToLocation = () => {
    axios.post(`https://maps.googleapis.com/maps/api/geocode/json?address=${this.state.currentAddress}&key=${googleAPIKey}`)
         .then((response)=>{
           ...
           axios.post('http://localhost:8080/currentposition', { latitude: response.data.results[0].geometry.location.lat, longitude: response.data.results[0].geometry.location.lng})
