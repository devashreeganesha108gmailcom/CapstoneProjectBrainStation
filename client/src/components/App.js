...
class Weather extends Component{
  ...
  generateGraphData = () => {
    if((this.state.graphParameter !== 'parameter' && this.state.graphParameter !== '') && (this.state.forecastRange !== 'range' && this.state.forecastRange !== '')){
      axios.post('https://weather-talker.herokuapp.com/generategraphdata', {graphParameter: this.state.graphParameter,forecastRange: this.state.forecastRange,data: this.state.data
      })
  ...
  getPictures = () => {
    axios.post('https://weather-talker.herokuapp.com/getPictures')
  ...
  convertAddressToLocation = () => {
    axios.post(`https://maps.googleapis.com/maps/api/geocode/json?address=${this.state.currentAddress}&key=${googleAPIKey}`)
         .then((response)=>{
           ...
           axios.post('https://weather-talker.herokuapp.com/currentposition', {
             latitude: response.data.results[0].geometry.location.lat,
             longitude: response.data.results[0].geometry.location.lng
           })
